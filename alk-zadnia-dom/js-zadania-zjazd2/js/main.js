...
function toTitleCase(toChangeSentence) {
    ...
    for (var i = 0; arraySentence.length > i; i++) {
        ...
        arraySentence[i] = arraySentence[i].substring(0, 1).toUpperCase() +
...
setInterval(function () { //wszystko co poniżej musi być w funkcji, aby funkcja                                  dziłała prawidłowo i aby strona w console.log odliczała co sekundę
...
function shuffle(arrayToChange) {
    ...
    for (var i = 0; arrayToChange.length > i; i++) {
        ...
        if (arrayToChange.length === randomArray.length) {
            ...
            randomNumberForIndeks = Math.floor(Math.random() * ghostArrayToChange.length);
...
function union(arrayOne, arrayTwo) {
    ...
    for (var j = 0; j < arrayNumberTwoGhost.length - 1; j++); {
        ...
        while (unionArray.length !== (arrayOne.length + arrayTwo.length)) {
            ...
            for (var n = 0; n < unionArray.length; n++) { //aby posortować muszą być dwie pętle --- to 1 Pętla

                for (var m = 0; m < unionArray.length - 1; m++) { // --- to 2 Pętla
...
console.log("Tablice liczb przed połączeniem: I Tablica [" + arrayNumberOne + "], II Tablica [" + arrayNumberTwo + "]");

console.log("Tablica po połączeniu i posortowaniu: [" + union(arrayNumberOne, arrayNumberTwo) + "]");

console.log("");

console.log("Tablice liter przed połączeniem: I Tablica [" + arrayLetterOne + "], II Tablica [" + arrayLetterTwo + "]");

console.log("Tablica po połączeniu i posortowaniu: [" + union(arrayLetterOne, arrayLetterTwo) + "]");
    ...
    while (arrayInterGhostOne.length > 0) {
        ...
        for (var j = 0; j < arrayInterTwo.length - 1; j++) {
            ...
            if (arrayInterGhostOne[i] === arrayInterTwo[j]) {
                ...
                for (var n = 0; n < interArray.length; n++) { //aby posortować muszą być dwie pętle --- to 1 Pętla

                    for (var m = 0; m < interArray.length - 1; m++) { // --- to 2 Pętla
...
console.log("Tablice, w których szukamy wspólnych wartości: I Tablica [" + arrayToInterOne + "], II Tablica [" + arrayToInterTwo + "]");

console.log("Tablica wartości wspólnych: [" + intersection(arrayToInterOne, arrayToInterTwo) + "]");
...
function secretNumber() {
    ...
    while (weNeedTrue === false) {
        ...
        if (searchingNumber = prompt("Aby dostać się na stronę wpisz szukaną wartość (z przedziału od 1 do 100)")) {
            ...
            if (searchingNumber == " ") {
                ...
                alert("Brak wpisania czegokolwiek, również nie załatwi sprawy! Proszę podać wartość od 1 do 100");
                ...
                } else if (secretRandomNumber == searchingNumber) { //Tutaj koniecznie '==', bo z '===' nie będzie działać.

                    if (tryNumber === 1) {

                        alert("Brawo! Potrzebowałeś tylko" + tryNumber + " próby by odgadnąć szukaną wartość");
                        ...
                        alert("Brawo! Potrzebowałeś: " + tryNumber + " prób by odgadnąć szukaną wartość");
                ...
                alert("Nawet nie będę tego komentował :(, Proszę podać wartość z zakresu od 1 do 100!");
