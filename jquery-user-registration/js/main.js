...
passwordInput.on({
    'keyup': function(keyPush) { //warto używać zdarzenia keyup --- najlepiej działa
        ...
        if (keyPush) {
            checkPassword($(this).val()); //this ---> jest (w tym miejscu) odpowiednikiem passwordInput ---> gdyż w zdarzeniach this wskazuje zawsze element (input, button, itp.), który dane zdarzenie wywołał>>> Tutaj np. wpisujemy w input hasła ---> litery, w ten sposób uruchamiając zdarzenie 'keyup'
